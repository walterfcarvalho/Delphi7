...
begin
   ...
   dsPed.DataSet := uPedCliente.getDadosPedCliente(nmpedido);
	...
	f.ajGridCol(SoftDBGrid1, dsPed.DataSet, 'operação', 100, 'Operação');
...
//   carregaItensPedido();
   ...
   begin
      msg.msgErro('Não achei esse pedido.');
      dsPed.DataSet := nil;
   end;
...
begin
   ...
   begin
      ...
      if (edValor.Value  > vDescAvarias + 0.10 ) then
         erro := erro + ' - O  desconto máximo, é de: R$ ' +  f.floatToMoney(vDescAvarias) + #13;
   end;
...
begin
   ...
   begin
         ...
         begin
            if msg.msgQuestion('Ja existe desconto nesse pedido.' +#13+ 'Se aplicar esse desconto o anterior irá ser descartado. '+#13+ 'Continua? ') = mrYes then
...
//   vPreco := '1';
   ...
   msg.msgErro('erro na alteração itens ');
...
// alterar os valores do pedido
  cmd :=  ' update pedidoCliente set ' +
   ...
   if (dm.openSQL(cmd, '@@error')  <> '0') then
      msg.msgErro('erro na alteração dos valores do pedido');
end;
...
begin
   ...
   if (criticaErroDesconto(vDesconto ) = '') then
   begin
      msg.msgExclamation('Verificação confirmada. ');
