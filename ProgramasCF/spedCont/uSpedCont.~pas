...
uses
  ...
  Dialogs, StdCtrls, Spin, Mask, adLabelEdit;
...
type
  TfmSpedCont = class(TForm)
    ...
    function getCnpj(cd_pes:string):String;
    function getDsParam(registro:String):TdataSet;
    function getItemFromNfe(anoMes, uo, cmd, fl:String;exclusao:TStrings; cnpjForn, di:String):String;
    function getItemFromXml(srvNfe, anoMes, uo, cmdSql, xml:String;exclusao:TStrings; salvaNoBd:boolean; cnpjForn:String):Tstringlist;
    ...
    procedure ajustac180Duplicado(var fl:TStringlist);
    ...
    procedure lerArquivosSPED(arq:String; exclusoes:TStrings; salvaNoBd:boolean);
    procedure recalculaC190(arq: String; perc:real);
...
uses uPessoa, fdt, uAcbr, rSped, f, msg, uAjustaSPED, uLj, udm, uMain, uProd;

{$R *.dfm}


function TfmSpedCont.getCnpj(cd_pes: string): String;
var
   dspes:TDataset;
begin
   dsPes :=
   uPessoa.getDadosPessoa(cd_pes);
   result := dsPes.fieldByName('CNPJ').AsString;
   dsPes.Free();
end;
...
function TfmSpedCont.getItemFromNfe(anoMes, uo, cmd, fl:String;exclusao:TStrings; cnpjForn, di:String):String;
var
   cfop, isRef, qt, vlUnd, vlTot, vlDesc:String;
begin
   ...
   dm.setParams(cmd, isRef,  quotedStr(cfop),   qt);
   dm.setParams(cmd, f.valorSql(vlUnd), f.valorSql(vlTot), f.valorSql(vlDesc));
   dm.setParams(cmd, quotedStr(cnpjForn), quotedStr(di), '' );
...
function TfmSpedCont.getItemFromXml(srvNfe, anoMes, uo, cmdSql, xml:String;exclusao:TStrings; salvaNoBd:boolean; cnpjForn:String):Tstringlist;
var
   arq:String;
   dsNf, dsTotIt,dsItNf, dsItNf_Di:TdataSet;
   fl :String;
   res:TStringList;
   di,is_ref:String;
   aux:String;
...
begin
   ...
   begin
     ...
     begin
        ...
        if (dsItNf.FieldByName('CFOP').AsString = '3102') then
        begin
           dsItNf_Di := uAcbr.getDaDosXml(arq, 'nfeEntItensDI.xtr');
           di := f.sohNumerosPositivos(dsItNf_Di.fieldByName('nDI').AsString);
           dsItNf_Di.Free();
        end;

        while (dsItNf.Eof = false) do
        begin
            f.gravaLog(dsItNf.RecNo);
           ...
           begin
              ...
              aux :=  fmSpedCont.getItemFromNfe(anoMes, uo, cmdSql, fl, exclusao, cnpjForn,  di);
              res.Add(aux);
           end;
           dsTotIt.Next;
           dsItNf.Next;
...
procedure TfmSpedCont.lerArquivosSPED(arq: String; exclusoes:TStrings; salvaNoBd:boolean);
var
   j:integer;
   fl:TStringList;
   anoMes, uo:String;

   tpEmisNf, cmd, str:String;
   statusNf, cnpjForn, cnpjUo:String;
   srvNfe:String;
   comandos, uosExcluir:TStringlist;
   dsPes :TdataSet;
   cdpesUo, l:String;
...
begin
   ...
   cnpjUo := fmSpedCont.getCnpj(uo);
   anoMes := copy(rSped.getCp( 4, fl[0]), 05,04) + copy(rSped.getCp( 4, fl[0]), 03,02);
   srvNfe := dm.getParamBD('ServerNFE.ip', uo);

   uosExcluir := dm.getListagem( dm.getCMD1('fiscal', 'uoIgnorar', uo), 0 );

   if (salvaNoBd = true ) then
      dm.execSQL(dm.getQtCMD2('fiscal', 'limpaMes', uo, anoMes));
   ...
   begin
      ...
      // notas emitidas por terceiros, apenas XML   {tipo emissao}                   {status '00' doc regular}
      if (rSped.getCp(1, fl[j]) = 'C100') and (rSped.getCp(3, fl[j]) = '0') and  ( rSped.getCp(6, fl[j]) = '00') then
      begin
         comandos := fmSpedCont.getItemFromXml(srvNfe, anoMes, uo, str, rSped.getCp(9, fl[j]), exclusoes, salvaNoBd, cnpjUo);
      ...
      // itens notas emissao propria

      if (rSped.getCp(1, fl[j]) = 'C100') and (rSped.getCp(3, fl[j]) = '1') and (rSped.getCp(6, fl[j]) = '00') then
         if (uosExcluir.IndexOf( rSped.getCp(4, fl[j]) ) < 0)    then
            cnpjForn := fmSpedCont.getCnpj( rSped.getCp(4, fl[j]) )
         else
         begin
            memo2.Lines.Add(fl[j]);
            cnpjForn := 'ignorar';
            memo2.Lines.Add('nf ignorada: '+  rSped.getCp(7, fl[j]) +'/'+rSped.getCp(4, fl[j]) +' uo:'+  rSped.getCp(4, fl[j]));
         end;

      if (rSped.getCp(1, fl[j]) = 'C170') and (cnpjForn <> 'ignorar') then
      begin
         cmd := getItemFromNfe(anoMes, uo, str, fl[j], exclusoes, cnpjForn, 'null');
      ...
      begin
         cmd :=  fmSpedCont.getItemFromReducao(anoMes, uo, str, fl[j], exclusoes);
         if (cmd <> '') then
            if (salvaNoBd = true) then
               dm.execSQL(cmd);
      end;
...
procedure TfmSpedCont.ajustac180Duplicado(var fl:TStringlist);
var
   rAnt, rAtu:String;
   idxAnt, idxAtu:String;
   linAux:String;
   vlIt1, vlIt2:real;
begin
   if fl.Count > 3 then
   ...
   if (rAnt = rAtu) and (rAnt <>  '') and ( rSped.getCp(1, fl[fl.count -8]) = 'C180') then
   begin
...
begin
   ...
   begin
      ...
      fmSpedCont.ajustac180Duplicado(fl);
   end;
...
var
   fl:TStringlist;
   dsIt, dsC190, dsC191, dsC195, dsC199:TdataSet;
   cmd:String;
   cfos:String;
begin
   ...
   dsC199:= dm.getDataSetQ(dm.getQtCMD1 ('fiscal', 'sped.getReg', 'c199'));
   ...
   cfos := dm.getcmd('Fiscal','cfo.entFor') +', '+
    ...
    begin
      ...
      begin
         ...
         f.addStringList(fl,  getDados(dsIt, dsC199, false, dsIt.FieldByName('cfo').AsString = '3102'));
...
begin
   ...
   campos := '|0000|0100|0110|0111|0140|0145|0150|0190|0200|0400|C010|C180|C181'+
             '|C185|C190|C191|C195|C199|C490|C491|C495|C500|C501|C505|F010|M100|M105'+
...
procedure TfmSpedCont.recalculaC190(arq: String; perc:real);
var
   fl:TStringlist;
   i:integer;
   vlItem:real;
...
begin
   ...
   begin
      if (rSped.getCp(1,  fl[i]) = 'C190') then
      begin
         vlItem := rSped.getVlCampo(8, fl[i]);
         vlItem := vlItem + (vlItem * perc);
         fl[i] :=   rSped.updateCampo(fl[i], 8, f.floatToStrFomatado(vlItem, true) );
      end;

      if (rSped.getCp(1,  fl[i]) = 'C191') or (rSped.getCp(1,  fl[i]) = 'C195') then
      begin
         vlItem := rSped.getVlCampo(5, fl[i]);
         aliquota := rSped.getVlCampo(08, fl[i]);

         vlItem := vlItem + (vlItem * perc);
         fl[i] :=   rSped.updateCampo(fl[i], 5, f.floatToStrFomatado(vlItem, true) );
         fl[i] :=   rSped.updateCampo(fl[i], 7, f.floatToStrFomatado(vlItem, true) );

         vImposto := vlItem * (aliquota/100);

         fl[i] :=   rSped.updateCampo(fl[i], 11, f.floatToStrFomatado(vImposto, true) );
      end;
   end;

   insert('_ajustado_', arq, length(arq)-3  );

   fl.SaveToFile(arq);
end;

procedure TfmSpedCont.Button4Click(Sender: TObject);
var
   arq:String;
begin
   arq := f.dialogAbrArq('txt', 'c:\');

   if (arq <> '') and ( f.sohNumeros(edvlPerc.Text) <> '') then
      recalculaC190(arq, strToFloat(edvlPerc.text));
end;
