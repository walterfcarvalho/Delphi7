...
uses printers, windows, dialogs, SysUtils, classes, MaskUtils, Registry,   unMsgTela2,
     ...
     adLabelMaskEdit, dbGrids, grids, uInput2, msg, uDirDialog,adLabelListBox;
       ...
       function addStringList(var listaBase:TStringList; lista:TStringList):TStringlist;
       ...
       function camposToStringlist(campos:String):Tstringlist;
       ...
       function floatToMoney(valor:Real):String; overload;
       ...
       function getCodAsc(Caracter: Char): String;
       ...
       procedure getComputadorUsuario(var estacao, usuario:String);
       function getCp(numCampo:integer; Objeto:Tobject): String;overload;
       function getCp(numCampo:integer; linha:String): String; overload;
       function getCpCombo(ds:Tdataset):TstringList;
       ...
       function getParamIni(secao, param:String):String; overload;
       function getParamIni(arq, secao, param:String):String; overload;

       function getParamsArIni(arIni:String):TStringlist;
       ...
       function gravaLinhaEmUmArquivo(nomeArq:String; ds:TdataSet; descDataSet:String ):boolean; overload;
		 function gravaLinhaEmUmArquivo(nomeArq:string; lst:TStringlist):boolean; overload;
       ...
       function isNumero(palavra:String; mostraMsg:boolean):boolean; overload;
       ...
       function NumLinArq(Arquivo: string): String;
		 function pontoPorVirgula(valor:String):String;
       ...
       function replace(cmd, ent, sai:String):String;
       ...
       function setUoNacomboBox(cb:TComboBox; uo:String):boolean;
       function sohLetras(str:string):string;
       function SohLetrasEspaco(str:string):string;
       ...
       function stringListToString(L:TstringList; separador:String):String;
       function stringListToCampos(lista:TStringlist):String;
       ...
       function getLista(lista:TStrings; isInicio:boolean):String;

       function valorSql(str:string):string; overload;
       function valorSql(str:string;truncaDecimais:boolean):string; overload;
       function valorSql(valor:Real):string; overload;
       function valorSql(valor:Real;truncaDecimais:boolean):string; overload;
       function valorIntSql(str:string):String;
       function MoneyXmlToADOMoney(valor:String):String;
       ...
       procedure salvaColunasDbGrid(grid: TSoftDbgrid; ds:TdataSet);
...
var
   posCentavo:integer;
begin
   posCentavo := pos('.', valor);

   delete(valor, posCentavo, 1);
   insert(',', valor, posCEntavo);
   result := valor;
end;
...
begin
   // retorna o arquivo texto default da aplicacao
	arq:=  'impressao_' + f.soLetrasNumerosPositivos(getNomeDaEstacao()) +'_' + '.txt';

   result := getDirLogs() + arq;
end;
...
begin
   if cbox.ItemIndex < 0 then
      ...
      result := f.preencheCAmpo(8,'0','d', sohNumeros(copy(cbox.Items[cbox.itemIndex],50,150)) );
end;
...
function valorSql(str:string):string; overload;
begin
	result := valorSql(str, true);
end;

function valorSql(valor:real):String; overload;
begin
   result := valorSQL(floatToStr(valor), false);
end;
...
begin
	aux := floatToStr(valor);
 	result := valorSql(aux, truncaDecimais);
end;
...
begin
   ...
   begin
      ...
      msg.msgErro('Erro ao ler o parâmetro, ' + nomeParam +#13+' Não encontrei o arquivo: '+ nomeArq);
end;
...
function limparLog():boolean;
begin
   result := DeleteFile( getArqLog() );
end;

function getDirExe():String;
begin
   // retorna no diretorio do executavel com "\" no fim
...
{
function gravaLog(valor:char):boolean; overload;
var
   aux:String;
begin
   aux := '<----- ' + valor ;
...
begin
   if nomeArq = '' then
      nomeArq := f.getDirLogs()+ '_gravalinhaEmArquivo.txt';
   ...
   try
      ...
      if fileExists(nomeArq) = false then
       ...
       begin
          f.gravaLog('Erro ao gravar arquivo: '+ nomeArq);
...
begin
	for i:=0 to lst.Count -1 do
   	gravaLinhaEmUmArquivo(nomeArq, lst[i]);
end;
...
var
   ...
   i:integer;
begin
   f.gravaLinhaEmUmArquivo(nomeArq, descDataSet + ' ---------------------------------------');

   ds.First();

   f.gravaLinhaEmUmArquivo(nomeArq, '{');
   ...
   begin
      ...
      f.gravaLinhaEmUmArquivo(nomeArq, cmd);
...
function remAcentos(str:String):String;
Const
   CE = 'áàãâäéèêëíìîïóòôöúùûüçÀÁÃÂÄÉÈÊËÍÌÎÏÓÒÔÖÚÙÛÜÇ~´`''"^';
...
begin
   ...
   if (s <> '') then
      ...
      gravaLog('Chave: ' + folder + ' Parametro: ' + Key + ' Resultado: ' + 'Não achei essa chave');
   RParReg:= s;
end;
...
begin
   ...
   result  := 'Provider=SQLOLEDB.1;Persist Security Info=False;' +
          ...
          arq[02] +';'+
          arq[03] +';'+
          'Workstation ID='+ f.getNomeDaEstacao() ;// GetNomeDoMicro();
...
//   f.GravaLinhaEmUmArquivo('c:\zCredito.txt',aux);
...
begin
   getComputadorUsuario(ComputerName, UserName);

   f.gravaLog('f.getNomeDaEstacao():'+ ComputerName +'_'+ UserName);

   Result := f.tiraEspaco(ComputerName +'_'+ UserName);
end;
...
begin //
   ...
   if (conf = false) then
     conf := (msgQuestion('Destino da impressão: '+#13 + nomePorta + #13+#13 + ' Prepare a impressora imprimir. Continua ?') = mrYes);
...
//      f.gravaLog(grid.Fields[i].FieldName +': '+  inttostr( grid.Columns[i].Width ));
end;
...
begin
   ...
   begin
//   	f.gravaLog(form.Components[i].Name + ' Classe:' + form.Components[i].ClassName);
// edits
      if form.Components[i].InheritsFrom(TcustomEdit) or (form.Components[i].ClassName = 'TadLabelEdit') then
          f.WParReg( 'ProgramasCF\'+ application.Title , form.Components[i].Name, (form.Components[i] as TcustomEdit).text);
// comboBox
      if form.Components[i].InheritsFrom(TCustomComboBox) then
         f.WParReg( 'ProgramasCF\'+ application.Title , form.Components[i].Name, IntToStr((form.Components[i] as TCustomComboBox).ItemIndex) );
// datePicker ou date
      if form.Components[i].InheritsFrom(TDateTimePicker)  or (form.Components[i].ClassName = 'TfsDateTimePicker') then
         f.WParReg( 'ProgramasCF\'+ application.Title , form.Components[i].Name  , dateToStr( (form.Components[i] as TDateTimePicker).date ) );
// pageControl
      if form.Components[i].ClassName = 'TPageControl'  then
         f.WParReg( 'ProgramasCF\'+ application.Title , form.Components[i].Name  , intToStr( (form.Components[i] as TPageControl).tabIndex ) );

      if (form.Components[i].ClassName = 'TFlatCheckBox') then
         f.WParRegBolean('ProgramasCF\'+ application.Title , form.Components[i].Name, (form.Components[i] as TFlatCheckBox).checked);

		if (form.Components[i].ClassName = 'TfsCheckBox') then
         f.WParRegBolean('ProgramasCF\'+ application.Title , form.Components[i].Name, (form.Components[i] as TfsCheckBox).checked);

//      if (form.Components[i].ClassName = 'TRadioGroup') then
//         f.wParRegint( 'ProgramasCF\'+ application.Title, form.Components[i].Name  , (form.Components[i] as TRadioGroup).itemIndex ) ;

      if (form.Components[i].ClassName = 'TSoftDBGrid') then
      begin
          for j:=1 to (form.Components[i] as TSoftDBGrid).columns.count -1 do
             f.WParRegint('ProgramasCF\'+ application.Title, form.Components[i].Name + '_C_'+intToStr(j), (form.Components[i] as TSoftDBGrid).Columns[j].Width );
      end;
   end;
  ...
  begin
     if (form.Width > 0) then
        f.WParRegint( 'ProgramasCF\'+ application.Title, form.name +'.width', form.Width );
     if (form.Top > 0) then
        f.WParRegint('ProgramasCF\'+ application.Title,  form.Name +'.top', form.top );
     if ( form.Left > 0) then
        f.WParRegint('ProgramasCF\'+ application.Title,  form.Name +'.left', form.left );

     if ( form.Height > 0) then
        f.WParRegint('ProgramasCF\'+ application.Title,  form.Name +'.height', form.Height );
  end;
end;
...
var
   ...
   aux:String;
begin
   gravaLog('Carregar informações do registro do form:' + form.Name);
   for i:=0 to form.ComponentCount-1 do
   begin
//   	f.gravaLog(form.Components[i].Name + ' Classe:' + form.Components[i].ClassName);
      if form.Components[i].InheritsFrom(TcustomEdit) or (form.Components[i].ClassName = 'TadLabelEdit') then
      begin
         aux := '';
         aux := f.RParReg('ProgramasCF\'+ application.Title, form.Components[i].Name);
      ...
      begin
         aux := '';
         if f.RParReg('ProgramasCF\'+ application.Title, form.Components[i].Name) <> '' then
         begin
            aux := f.RParReg('ProgramasCF\'+ application.Title, form.Components[i].Name);
      ...
      begin
         ...
         begin
            aux := f.RParReg('ProgramasCF\'+ application.Title, form.Components[i].Name);
            (form.Components[i] as TFlatCheckBox).Checked := f.RParRegBool('ProgramasCF\'+ application.Title, form.Components[i].Name);
         end;
      end;
      ...
      begin
         ...
         begin
            aux := f.RParReg('ProgramasCF\'+ application.Title, form.Components[i].Name);
           (form.Components[i] as TfsCheckBox).Checked := f.RParRegBool('ProgramasCF\'+ application.Title, form.Components[i].Name);
         end;
      end;
...
//      if (form.Components[i].ClassName = 'TRadioGroup') then
//         (form.Components[i] as TRadioGroup).ItemIndex := 0; //f.rParRegInt('ProgramasCF\'+ application.Title, form.Components[i].Name);
	end;

   if (form.WindowState <> wsMaximized) then
    begin
       if (f.RParRegInt('ProgramasCF\'+ application.Title, form.Name +'.width') > 0 ) then
          form.Width :=  f.RParRegInt('ProgramasCF\'+ application.Title, form.Name +'.width');

       if (f.RParRegInt('ProgramasCF\'+ application.Title, form.Name +'.top') > 0) then
          form.top :=  f.RParRegInt('ProgramasCF\'+ application.Title, form.Name +'.top');

       if ( f.RParRegInt('ProgramasCF\'+ application.Title, form.Name +'.left') > 0) then
          form.left :=  f.RParRegInt('ProgramasCF\'+ application.Title, form.Name +'.left');
...
//        f.gravaLog( 'ultdig atual:'+ ultDigAtual );
...
//    f.gravaLog('arredondar para ' + ultDig);
...
function getCodPc(cb:TCustomComboBox):String;
begin
   result := f.SohNumeros(copy(cb.Items[cb.ItemIndex],50,50));
end;
...
begin
   ...
   begin
      ...
      f.gravaLog('Deletar arquivo upgrade: ' + sysUtils.boolToStr(acao, true) );
      acao := renameFile( extractFilePath(paramStr(0)) + arqNovo,  extractFilePath(paramStr(0)) + arqAntigo );
      f.gravaLog('Renomear ' + arqNovo + ' para ' + arqAntigo +': ' + sysUtils.boolToStr(acao, true) );
   end;
end;
...
begin
   try
      ...
      on e:exception do
      begin
         msg.msgErro('Houve um erro ao gravar o parâmetro '+ parametro);
         result := false;
      end;
   end;
end;
...
begin
   try
      ...
      on e:exception do
      begin
         msg.msgErro('Houve um erro ao ler o parâmetro '+ parametro);
         result := '';
      end;
   end;
end;
...
begin
   try
      f.gravaLog('f.isEan13():' + cod);
      cod := sohNumeros(cod);
      res := false;
      if ( sohNumeros(cod) <> '') then
        if (length(cod) >= 13) then
        begin
           aux := copy(cod, 01,  length(cod)-1);
           aux := aux + f.getDigVerEAN13(aux);
           res:= (aux = cod);
        end;
   except
      res := false;
...
function getIniFile(arq:String):TiniFile;
begin
   if (arq = '') then
      arq:= f.getDirExe() + getNomeDoExecutavel() + '.ini';

   result := TIniFile.Create(arq);
end;
...
begin
   ...
   gravaLog('Lendo parâmentro ini, arq:' + arq + ' seção:' + secao + ', chave: '+ param + ', Resultado: '+ res );

   if (res = '') then
     gravaLog('parâmetro não encontrado, seção:' + secao + ', chave: '+ param + ', Resultado: '+ res );
...
begin
   result := getParamIni('', secao, param);
end;
...
begin
   ...
   if (mostraMsg = true) and (ehNum = false) then
      msg.msgErro( palavra + ' não é um número.');
...
begin
   ...
   script :=   f.getDirExe()+'execAndWait.bat';
...
// encapsular o comando em um arquivo.bat
   ...
   begin
      arq.Add(comandos[i]);
      f.gravaLog(comandos[i]);
   end;

{   if (f.getParamIni('conexao', 'showScript') = '1') then
   begin
...
begin
   ...
   begin
      if (isInicio = true) then
      begin
         for i:=0 to lista.Count -2 do
            aux := aux + trim(lista[i]) + ', ';
         aux := aux +  trim(lista[lista.Count-1]);
      end
      else
      begin
         for i:=0 to lista.Count -2 do
            aux := aux + trim( copy(lista[i], 101, 20)) + ', ';
         aux :=  aux + trim(copy(lista[lista.Count-1], 101, 20));
      end;
   end
   else
      aux := '';
...
var
   ...
   res:boolean;
begin
//	f.gravaLog('dataSetTemCampo()' + campo);
...
var
   linha:String;
begin
   f.gravalog (objeto.ClassName);

   if (objeto.InheritsFrom(TCustomComboBox) = true) then
      linha := (objeto as TCustomComboBox).Items[(objeto as TCustomComboBox).ItemIndex];

   result:= getcp(numCampo, linha);
end;

function getCp(numCampo:integer; linha:String): String; overload;
var
   posInicio, tamCampo:integer;
   separador:char;
begin
   ...
   posInicio := f.getPosInicio(numCampo, linha);

   tamCampo := 0;
   while ((tamCampo+posInicio) <= length(linha)) and ( linha[posInicio+tamCampo] <> separador) do
      inc(tamCampo);

   linha := trim(copy(linha, posInicio, tamCampo));

   if (length(linha) > 0) then
      if(linha[length(linha)] = separador) then
...
var
   ...
   res:Tstringlist;
begin
   // recebe campos do formato ||||||
   // e devolce uma stringlist
...
begin
   ...
   while ( ds.Eof = false) do
   begin
      aux := f.preencheCampo(100, ' ', 'D',  ds.Fields[0].AsString);
      for i:=1 to ds.FieldCount-1 do
         aux := aux + '|'+ ds.Fields[i].AsString;
...
begin
   ...
   e.LoadFromFile(arIni);
   p := TStringlist.Create;
   for i:=0 to e.Count -1 do
   begin
      if ( pos('=', e[i]) > 0) then
         P.Values[ trim( copy(e[i], 01, pos('=', e[i]) -1)) ] := trim( copy(e[i], pos('=', e[i])+1, length(e[i])));
   end;
   e.Free();
//   f.gravaLog(p);
   result := p;
...
var
   i:integer;
begin
   if (listaBase = nil) then
      listaBase := TStringlist.Create();

   if (lista <> nil) then
      for i:=0 to lista.Count -1 do
         listaBase.add(lista[i]);
