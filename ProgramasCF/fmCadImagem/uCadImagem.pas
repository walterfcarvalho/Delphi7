...
type
  TfmCadastro = class(TForm)
    ...
    procedure btRenomeiaClick(Sender: TObject);
    procedure dirListBoxChange(Sender: TObject);
    procedure renArqImg(dir:String);
    procedure renomear(arq:String);
...
var
  ...
  cod:String;
begin
   f.gravaLog('btExportarImgClick()');
	ds:= dm.getDataSetQ( dm.getCMD1('estoque', 'prdComImg', edFaixa.Text));

   ds.First;
   while ( ds.Eof =  false ) do
   begin
      memo1.Lines.Add(ds.fieldByName('is_ref').asString + ' ' + ds.fieldByName('cd_ref').asString);

      fmMain.msgStatusProgresso(ds.RecNo, ds.RecordCount, '' );
...
// colocar o codigo do protheus
      ...
      if (Image1.Picture <> nil) then
      begin
        cod := f.getDirLogs()+ '\Imagens\' + cod + '.bmp';
        f.gravaLog('Arquivo a salvar: '+ cod);
        Image1.Picture.SaveToFile( cod );
        FmMain.converterBmpParaJPeg(cod)
      end
      else
...
procedure TfmCadastro.dirListBoxChange(Sender: TObject);
begin
   FileListBox1.Directory := dirListBox.Directory;
end;

procedure TfmCadastro.renArqImg(dir: String);
var
   i:integer;
   arq:String;
begin
   ...
   begin
      arq := dir +'\'+ FileListBox1.Items[i];
      renomear(arq);
   end;
end;

procedure TfmCadastro.renomear(arq:String);
var
   novoArq, path, nmArq, ext:String;
begin
   f.gravaLog('renomear-> ' + arq);

  path := ExtractFileDir(arq);
  nmArq :=  extractFileName(arq);
  ext :=  ExtractFileExt(arq);

  if length(nmArq) = 7 then
     nmArq := '8'+ nmArq;

  insert('00001', nmArq, length(nmArq)-3);
//  nmArq := nmArq + '00001';
  novoArq :=  path +'\'+ nmArq;

  if ( UPPERCASE(ext) = '.JPG')  and (length(nmArq) <= 17)  then
     RenameFile(arq, novoArq);
end;

procedure TfmCadastro.DriveComboBox1Change(Sender: TObject);
begin
   dirListBox.Drive := DriveComboBox1.Drive;
end;

procedure TfmCadastro.FormShow(Sender: TObject);
begin
   DriveComboBox1.Drive := 'e';
end;

procedure TfmCadastro.btRenomeiaClick(Sender: TObject);
var
   i:integer;
begin
   for i:=0 to memo1.Lines.Count - 1 do
   begin
      FileListBox1.Directory := memo1.Lines[i];
      FileListBox1.Refresh();
      renArqImg(FileListBox1.Directory);
   end;
end;
