...
uses
  ...
  ExtCtrls, adLabelNumericEdit, uHash, uPessoa;
...
type
  TfmAjustaSPED = class(TForm)
    ...
    cbRecC100prop: TfsCheckBox;
    cbRecC100ter: TfsCheckBox;
    cbRemNfEntEmissPropria: TfsCheckBox;
    edRecC100prop: TadLabelEdit;
    ...
    function getQtCampos(linha:string):integer;
    ...
    function removeNfEntFornIgualLoja(fl:TStringlist):Tstringlist;
    ...
    function recC100_terceiro(fl:TStringlist):TStringlist;
    function recC100_proprio(fl:TStringlist):TStringlist;
    function getHash(f:TsTringlist; valorCampo:String; idxHash, idxValor:integer):Thash;
    function getIdxRegraAjC100(cfo, ncm, origem, cst, uf:String; regras:TStringlist):integer;
    function pegaUfHashReg150(hashP:Thash): Thash;
    function getHashProd(f:TStringlist):Thash;
    function getHashPessoa(f:TStringlist):Thash;
  private
...
function TfmAjustaSPED.getHash(f:TsTringlist; valorCampo:String; idxHash, idxValor:integer):Thash;
var
   hash:Thash;
   i:integer;
begin
   ...
   begin
      if getcp( 1, f[i]) = valorCampo  then
         Hash.add(getcp(idxHash, f[i]), getcp(idxValor, f[i]))
   end;
   result := Hash;
end;


function TfmAjustaSPED.getHashProd(f:TStringlist):Thash;
var
   h:Thash;
   i:integer;
   ds:TdataSet;
...
begin
   ...
   ds:= dm.getDataSetQ('select distinct is_ref, ncm_sh, tp_origem from crefe where is_ref in (' + cmd + ')');

   ds.First();
   while ds.Eof = false do
   begin
      h.addAll(ds.fieldByName('IS_REF').asString, ds.fieldByName('NCM_SH').asString, ds.fieldByName('TP_ORIGEM').asString, '', '');
      ds.Next()
   end;
   ds.Free();
   result := h;
...
function TfmAjustaSPED.getHashPessoa(f:TStringlist):Thash;
var
   h:Thash;
   i:integer;
   ds:TdataSet;
...
begin
   ...
   ds:= dm.getDataSetQ('select distinct cd_pes, cd_uf from dspes where cd_pes in ('+cmd+ ')') ;

   ds.First();
   while ds.Eof = false do
   begin
      h.addAll(ds.fieldByName('cd_pes').asString, ds.fieldByName('cd_uf').asString, '', '', '');
      ds.Next();
   end;
   ds.Free();
   result := h;
...
function TfmAjustaSPED.pegaUfHashReg150(hashP:Thash): THash;
var
   i:integer;
begin
   for i:=0 to hashP.count -1 do
   begin
      hashP.setValorIdx(i, uPessoa.getUfPessoa( hashP.getIndex(i) ));
   end;
   result := hashP;
end;

function TfmAjustaSPED.getIdxRegraAjC100(cfo, ncm, origem, cst, uf:String; regras:TStringlist):integer;
var
  lOk:boolean;
  i:integer;
  hashP:Thash;
  cfop_regra, ncmRegra, origem_regra, cst_regra, uf_regra:String;
  idxregra :integer;
begin
   lOk := false;
   ...
   if ncm <> '4202' then
      ncm := '*';
//   f.gravaLog('---------------------------------------------------' ) ;
//   f.gravaLog('cfo:'+ cfo + '  ncm:'+ncm+ ' origem:'+ origem + ' cst:'+ cst+ ' uf:'+ uf ) ;

   idxregra := -1;
   for i:=1 to regras.Count -1 do
   begin
      cfop_regra := getcp(1, regras[i]);
      ncmRegra := getcp(2, regras[i]);
      origem_regra := getcp(5, regras[i]);
      cst_regra:= getcp(6, regras[i]);
      uf_regra:=  getcp(8, regras[i]);

//      f.gravaLog('regra->cfop:'+ cfop_regra + '  ncm:'+ncmRegra+ ' origem:'+ origem_regra + ' cst:'+ cst_regra+ ' uf:'+ uf_regra ) ;

      if (cfo = cfop_regra)                                  and
         (ncm = ncmRegra)                                    and
         ((pos(origem, origem_regra) >0) or (origem_regra ='*' ))   and
         ( pos(cst, cst_regra) > 0)                         and
         ( pos(uf, uf_regra) > 0 )      then
      begin
         idxregra := i;
//         f.gravaLog('regra:'+ intToStr(i));
         break;
      end;
//      if idxregra = -1 then
   end;
...
function TfmAjustaSPED.recC100_terceiro(fl: TStringlist): TStringlist;
var
  j, i, h:integer;
  regras:Tstringlist;
  idxRegra:integer;
  aux, percIcms, uf, cfo, ncm_sh, tp_origem, vlBase, cst, is_ref:String;
  hashT, hashProd, hashPessoa:Thash;

  vBase, vImposto, vIcmsItem:real;
begin
   memo.Lines.Add('Carregando as regras...(regraRecC100.csv)');
   if FileExists( f.getDirExe()+ 'regraRecC100.csv') = false then
   begin
      msg.msgErro(dm.getMsg('errsped001'));
      exit;
   end;
   regras:= Tstringlist.Create();
   regras.LoadFromFile('regraRecC100.csv');

   hashT := THash.Create();

   memo.Lines.Add('Listando os dados dos produtos...');
   hashProd := getHashProd(fl);

   memo.Lines.Add('Listando os dados dos participantes...');
   hashPessoa := getHashPessoa(fl);

   memo.Lines.Add('Recalculando dados da C170');

   i:= 10;

   while i < fl.Count -1 do
   begin
      if (getcp(1, fl[i]) = 'C100') and (getcp(3, fl[i]) = '1') then
      begin
         f.gravaLog('Nota:'+ fl[i]);
         uf := hashPessoa.getValor(getcp(4, fl[i]));

         j:= i+1;
         while (getcp(1, fl[j]) = 'C170') do
         begin
            is_ref := getcp(03, fl[j]);
            cfo := getcp(11, fl[j]);
            tp_origem := hashProd.getValor2(is_ref);
            ncm_sh := copy( hashProd.getValor(is_ref), 01, 04);

            if ncm_sh = '' then
            begin
               f.gravaLog('item sem ncm:' + ncm_sh);
            end;


            cst := copy(getcp(10, fl[j]), 02, 02);
            idxRegra := getIdxRegraAjC100(cfo, ncm_sh, tp_origem, cst, uf, regras);

            if idxRegra > -1 then
            begin
               // pega novamente o cfo e o cst
               cfo := getcp(4, regras[idxRegra]);
               percIcms:= getcp(3, regras[idxRegra]);
               cst := copy(getCp(10, fl[j]),01,01) + getcp(7, regras[idxRegra]);
               fl[j]:= updateCampo(fl[j], 11, cfo);
               fl[j]:= updateCampo(fl[j], 14, percIcms);
               fl[j]:= updateCampo(fl[j], 10, cst);

               if percIcms = '0' then
                  vlBase := '0'
               else
                  vlBase := getCp(13, fl[j]);

               if vlBase = '' then
               begin
                   memo.lines.add('Produto sem base de calculo, linha ' + intToStr(j+1) + ' campo 13'  );
                   vlBase := '0'
               end;

               vIcmsItem := strToFloat(vlBase) * (getVlCampo(14, fl[j]) / 100);
               fl[j]:= updateCampo(fl[j], 15, f.FloatToStrFomatado(vIcmsItem, true) );
               hashT.somaValores('|'+cst+'|'+cfo+'|'+percIcms+'|', strToFloat(getCp(7, fl[j])), strToFloat(vlBase), vIcmsItem, getVlCampo(24, fl[j]));
            end
            else
               memo.Lines.Add('Sem regra reg linha:' +  intToStr(j+1) + ' cfo:' + cfo + ' ncm:'+ ncm_sh + ' origem:'+ tp_origem + ' cst:'+ cst + ' uf:'+ uf );
            j:= j+1;
         end;

         while (getcp(1, fl[j]) = 'C190') and (hashT.count > 0) do
            fl.Delete(j);

         vImposto:=0;
         vbase := 0;
         for h:=0 to hashT.count -1 do
         begin
            cst :=      getcp(01, hashT.getVlIdx(h));
            cfo :=      getcp(02, hashT.getVlIdx(h));
            percIcms := getcp(03, hashT.getVlIdx(h));

            if strTofloat(percIcms) >0 then
            begin
               vBase := vBase + strToFloat(HashT.getVl2(h));
               vImposto:= vImposto + strToFloat(HashT.getVl3(h));
            end;

            aux:= '|' + 'C190'+'|' + cst+'|'+cfo+'|' + percIcms + '|' + HashT.getVl(h)  + {valor operacao}'|' +
                  HashT.getVl2(h) + {valor base}    '|' +HashT.getVl3(h) + {valor do icms} '|0|0|0|'+HashT.getVl4(h) + {valor IPI} '||' ;
         ...
         begin
            hashT.saveToLog();
            fl[i]:= updateCampo(fl[i], 21, f.FloatToStrFomatado(vBase, true)  {valor base calc icms} );
            fl[i]:= updateCampo(fl[i], 22, f.FloatToStrFomatado(vImposto, true) );
         end;
         hashT.clear();
      end;
      inc(i);
...
function TfmAjustaSPED.recC100_proprio(fl: TStringlist): TStringlist;
var
   ds:TdataSet;
   uo, cmd, aux, cst, cfo, pcIcm, cBase :String;
   hashT:THash;
   iC90, i, j, h, idxRegra:integer;
   regras:Tstringlist;
   vlBase, vlIcms, vImposto, vBase:Real;
begin
{
   recalcula os valores das notas de emissao propria
   usando oarquivo de regras.
}
   if FileExists( f.getDirExe()+ 'regraRecC100.csv') = false then
   begin
      msg.msgErro(dm.getMsg('errsped001'));
   ...
   hashT := THash.Create();
   uo := uLj.getIsUoFromCNPJ( getcp(7, fl[0]));

   if edRecC100prop.Text <> '' then
      uo := uo + ', '+ edRecC100prop.Text;

   i := 10;

   while i < fl.Count -1 do
   begin
      fmMain.msgStatusProgresso(i, fl.Count-2, '->');

      if (getcp(1, fl[i]) = 'C100') and (getcp(2, fl[i]) = '1' {saida}) and (getcp(3, fl[i]) = '0'{propria}) then
      begin
         // pega os itens
         cmd := dm.getCMD3('fiscal','lstItSped1', uo, QuotedStr(getcp(7, fl[i])), getcp(8, fl[i]) );
         ds := dm.getDataSetQ(cmd);
// recalcular o registro c190 na memoria
         ds.First();
         while ds.Eof = false do
         begin
            if ds.fieldByName('ncm_sh').AsString = '' then
            begin
               f.gravaLog('item sem ncm:' + ds.fieldByName('ncm_sh').AsString);
            end;

            idxRegra :=
            getIdxRegraAjC100( ds.fieldByName('codCfo').AsString, copy(ds.fieldByName('ncm_sh').AsString, 01, 04),
                               ds.fieldByName('TP_ORIGEM').AsString, copy(ds.fieldByName('cst').AsString, 02, 02),
                               ds.fieldByName('cd_uf').AsString, regras);
            if idxRegra > -1 then
            begin
               cfo  := getcp(4, regras[idxRegra]);
               pcIcm:= getcp(3, regras[idxRegra]);

               if strToFloat(pcIcm) = 0 then
                  vlBase := 0
               else
                  vlBase := ds.fieldByName('valorBaseIcms').AsFloat;

               cst := copy( ds.fieldByName('cst').AsString, 01,01) + getcp(7, regras[idxRegra]);
               vlIcms := ds.fieldByName('valorBaseIcms').AsFloat * (strToFloat(pcIcm) /100);

               hashT.somaValores('|'+cst+'|'+cfo+'|'+pcIcm+'|',
                                 ds.fieldByName('vl_item').AsFloat,
                                 vlBase,
                                 vlIcms,
                                 ds.fieldByName('VALORIPI').AsFloat,
                                );
            end
            else
            begin
               memo.Lines.Add('recC100_proprio sem regra reg linha:' +  intToStr(j+1) + ' cfo:' + cfo + ' ncm:'+
                              copy(ds.fieldByName('cst').AsString, 02, 02) + ' origem:'+ ds.fieldByName('TP_ORIGEM').AsString +
                              ' cst:'+ cst + ' uf:'+ ds.fieldByName('cd_uf').AsString);

               cmd := '|' +  DS.fieldByName('CST').asString +
                      '|' +  DS.fieldByName('CodCFO').asString +
                      '|' +  DS.fieldByName('PC_ICM').asString + '|';
               hashT.somaValores(CMD,
                                 ds.fieldByName('vl_item').AsFloat,
                                 ds.fieldByName('valorbaseIcms').AsFloat,
                                 ds.fieldByName('valorIcms').AsFloat,
                                 ds.fieldByName('VALORIPI').AsFloat,
                                );
            end;
            ds.Next();
         end;
// deletar os registros  do c190
         iC90:= i+1;
         while (getcp(1, fl[iC90]) = 'C190') and (hashT.count >  0) do
            fl.Delete(iC90);
// refazer o registro c190
         ...
         begin
            cst := getcp(01, hashT.getVlIdx(h));
            cfo := getcp(02, hashT.getVlIdx(h));
            pcIcm := getcp(03, hashT.getVlIdx(h));
            ...
            aux:= '|' + 'C190'+'|' + cst+'|'+cfo+'|' + pcIcm + '|' +
                  HashT.getVl(h)  + {valor operacao}'|' +
                  HashT.getVl2(h) + {valor base}    '|' +
                  HashT.getVl3(h) + {valor do icms} '|0|0|0|'+
                  HashT.getVl4(h) + {valor IPI}     '||' ;

               fl.Insert(iC90, aux);
//            f.gravaLog(' nota propria, linha:'+ inttoStr(j) + ' - ' + aux );
...
function TfmAjustaSPED.getQtCampos(linha:string):integer;
var
  i, qt, posInicio, tamanho :integer;
begin
{   qt:=0;
   for i:=0 to len(linha)-1 do
      if linha[i] = '|' then
         inc(qt); }
   result := qt-1;
end;
...
function TfmAjustaSPED.removeNfEntFornIgualLoja(fl:TStringlist):Tstringlist;
var
   i:integer;
   uo:String;
   hashP : Thash;
begin
   ...
   memo.Lines.Add('Procurando notas de terceiro que o fornecedor é o mesmo cd_pes do emitente.');
   i:=0;
   while i < fl.Count -1 do
   begin
      fmMain.msgStatusProgresso(i, fl.Count-1, '->');
      if (getcp(1, fl[i]) = 'C100') and (getcp(3, fl[i]) = '1') and (getcp(4, fl[i]) = uo) then
      begin
         memo.Lines.add(' nota: ' + getcp(8, fl[i]) );

         fl.Delete(i);

         while (getcp(1, fl[i]) = 'C170') do
         begin
            hashP.somaValores(getcp(3, fl[i]), 1, 1, 1, 1);
            fl.Delete(i);
         end;

         while (getcp(1, fl[i]) = 'C190') do
            fl.Delete(i);
      end;
      inc(i);
   end;
   ...
   memo.Lines.Add('Removendo itens da C200.');
   i:= 1;
   while i < fl.Count -1 do
   begin
      if (getcp(1, fl[i]) = '0200') and (hashP.indexOf(getcp(2, fl[i])) > -1) then
      begin
         memo.Lines.Add('Removendo produto...' +   getcp(2, fl[i]) );
         fl.delete(i);
      end;
      inc(i);
   end;
...
begin
   ...
   if (cbRecC100ter.Checked = true ) then
      arq := recC100_terceiro(arq);

   if (cbRecC100prop.Checked = true ) then
      arq := recC100_proprio(arq);

   if (cbRemNfEntEmissPropria.Checked = true) then
      arq:= removeNfEntFornIgualLoja(arq);
