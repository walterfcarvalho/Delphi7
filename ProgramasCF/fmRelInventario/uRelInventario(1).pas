...
begin
    ...
    aux.Add(arquivos[0]);
    carregaTbInventario(tbItens, aux, true, tbErr);
    f.gravaLog('carreguei  a primeira');

    aux.Clear();

    aux.Add(arquivos[1]);
    carregaTbInventario(tbNmArquivos, aux, true, tbErr);
    f.gravaLog('carreguei  a segunda');
...
begin
   ...
   begin
      ...
      begin
         ...
         memo1.Refresh(); fmRelInventario.Refresh();
         ...
         begin
            ...
            begin
               ...
               begin
                  ...
                  addErro('Não encontrado: ' +  lst[i] +' '+ arquivos[j] + ' linha:'+ intToStr(i), tbMsgErr);
               dsItem.Free();
            end
            else
...
begin
   fmMain.msgStatus('Calculando divergencias...');
    ...
    begin
       ...
       cmd := 'select * from '+ tbNmArquivos.TableName +' where  codigo = ' +
...
begin
   ...
   if (arquivos <> nil) and (arquivos.Count = 2) then
      ...
      begin
         ...
         if fsJeitoNovo.Checked = false then
            montaRelDivergencia(arquivos)
         else
            geraTbDirvegencia(arquivos);
...
var
  ...
  dsItem:TdataSet;
begin
   msg.msgExclamation('Juntar os arquivos na table zcf_juntaContLoja');
